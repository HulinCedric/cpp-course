//
// IUT de Nice / Departement informatique / Module APO-C++
// Annee 2008_2009 - Package _Maths
//
// Classe Fraction - Tests unitaires des operateurs == et != (cas nominaux)
//
// Auteur : C.Fouco
//
#include "Fraction.h"
#include "T:\_Tests\Tests\Tests.h"

void main () {

   Tests::Begin("_Maths.Fraction", "2.3.0");

   Tests::Design("Controle des operateurs (Cinquieme partie)", 3);
	
      Tests::Case("Operateur == : comparaison de deux fractions"); {
      
      Fraction f1(2,3),  f2(7,5), f3(-2,-3), f4(2,3);
      Fraction f5(2,-3), f6(-4,6), f7(1,3), f8(1,-3);
      Fraction f9(-4,5), f10(8,-10);

         Tests::Unit(0,  f1==f2);
         Tests::Unit(1,  f3==f4);
         Tests::Unit(1,  f5==f6);
         Tests::Unit(0,  f7==f8);
         Tests::Unit(1,  f9==f10);
      }

      Tests::Case("Operateur == : comparaison avec un entier"); {
      
      Fraction f1(2,4),  f2(-28,14), f3(0,-3), f4(-2,-1);

         Tests::Unit(0,  f1==2);
         Tests::Unit(1,  f2==-2);
         Tests::Unit(1,  f3==0);
         Tests::Unit(0,  f4==-1);
      }

      Tests::Case("Operateur != : comparaison de deux fractions"); {
      
      Fraction f1(2,3),  f2(7,5), f3(-2,-3), f4(2,3);
      Fraction f5(2,-3), f6(-4,6), f7(1,3), f8(1,-3);

         Tests::Unit(1,  f1!=f2);
         Tests::Unit(0,  f3!=f4);
         Tests::Unit(0,  f5!=f6);
         Tests::Unit(1,  f7!=f8);
      }
      Tests::Case("Operateur != : comparaison avec un entier"); {
      
      Fraction f1(2,4),  f2(-28,14), f3(0,-3), f4(-2,-1);

         Tests::Unit(1,  f1!=2);
         Tests::Unit(0,  f2!=-2);
         Tests::Unit(0,  f3!=0);
         Tests::Unit(1,  f4!=-1);
      }

   Tests::End();
}
