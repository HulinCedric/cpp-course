...
//
// Classe Ensemble - Tests unitaires de l'operateur += et 
//                   du constructeur de copie (Cas nominaux)
//
// Auteur : A. Thuaire
//
#include "Ensemble.h"
...
void main () {
   ...
   Tests::Begin("_Maths.Ensemble", "1.0.0");
      ...
      Tests::Design("Controle de l'operateur +=", 3); 
         ...
         Ensemble E1(&fEntierEquals);

            E1 += new int(2);

            Tests::Unit("{2}", E1.toString(&fEntierToString));
         }
         ...
         Ensemble E1(&fEntierEquals);
            ...
            Tests::Unit("{2, 4, 6}", E1.toString(&fEntierToString));
         }

         Tests::Case("Tentative d'ajout d'un element daja present"); {
         Ensemble E1(&fEntierEquals);
      ...
      Tests::Design("Controle du constructeur de copie", 3);
	  
         Tests::Case("Cas particulier de l'ensemble vide"); {
         Ensemble E1, E(E1);
         ...
         Ensemble E1(&fEntierEquals, &fEntierNew);
            ...
            Ensemble* pCopie= new Ensemble(E1);

            Tests::Unit("{2, 4, 6}", pCopie->toString(&fEntierToString));
         }
