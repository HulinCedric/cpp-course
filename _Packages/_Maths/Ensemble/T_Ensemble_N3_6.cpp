...
void main () {
   ...
   Tests::Begin("_Maths.Ensemble", "1.0.0");
      ...
      Tests::Design("Controle de l'operateur - / Retrait d'un element", 3); 
         ...
         Ensemble* pResultat;
            ...
            Tests::Unit("{4, 6}", pResultat->toString(&fEntierToString));

            pResultat= *pResultat - new int(6);
            Tests::Unit("{4}", pResultat->toString(&fEntierToString));

            pResultat= *pResultat - new int(4);
            Tests::Unit("{}", pResultat->toString(&fEntierToString));
         }

      Tests::Design("Controle de l'operateur - / Soustraction d'un ensemble", 3); 
         ...
         Ensemble* pResultat;
            ...
            pResultat= Op1 - Op2;
            Tests::Unit("{2, 1}", pResultat->toString(&fEntierToString));
         }

         Tests::Case("Cas particulier d'un resultat vide"); {
         Ensemble Op1(&fEntierEquals, &fEntierNew);
         Ensemble* pResultat;
